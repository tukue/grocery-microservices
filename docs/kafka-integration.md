...
| `order.created.v1.failed` | Operations | Retain records that exhaust summary consumer retries for investigation and controlled replay. |
...
## Consumer Retries and Failed-Letter Topic
...
The failed-letter topic is deliberately not consumed automatically. Inspect the record, correct the underlying issue, then replay it with an explicit operator action. This prevents poison messages from repeatedly blocking the main consumer group.
