...
- Define dashboards and alerts for HTTP 5xx rate, latency, unavailable dependencies, order-event-store `FAILED` records, consumer lag, and failed-letter queue growth.
...
- [ ] Kafka is managed, secured, monitored, and has a named owner for failed-letter queue replay.
