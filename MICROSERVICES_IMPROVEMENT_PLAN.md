...
- [x] **Catalog-Backed Cart Items**: Cart additions now require a product ID and fetch the catalog snapshot (name, price, availability) from `product-service`. Missing products return `404`, out-of-stock products return `409`, and catalog outages return `503`. (Covered by cart and product service/controller tests.)
